...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(32, 1, 1) # Final layer to output a single-channel heatmap
        )
...
def test_model(model, dataloader, device, model_loss_type='l1', visualization_dir=None, num_random_visualizations=3):
    ...
    with torch.no_grad():
        for i, (original_images_tensor_for_display, images_for_model, ground_truth_heatmaps, img_paths) in enumerate(tqdm(dataloader)):
            ...
            # --- Collect Visualization Data Candidates ---
            if visualization_dir: # Only collect if visualization is requested
                for j in range(images_for_model.size(0)):
                    current_img_mae = batch_individual_mae_tensors[j].item()
                    current_img_original_path = img_paths[j] # Use original path for uniqueness
                    ...
                    if len(random_viz_candidates) < (num_random_visualizations + 5): 
                        
                        if current_img_original_path not in [item['path'] for item in random_viz_candidates] and \
...
def save_metrics_plot(metrics_df, output_plot_path):
    ...
    ax.set_title('3D Heatmap MAE Comparison Across SimpleCNN Models', fontsize=16, weight='bold')

    # Adjust view angle for better visualization (optional)
...
if __name__ == "__main__":
    ...
    #MODEL LOADING
    MODEL_CONFIGS = {
        'SimpleCNN_L1Loss_Model': {
            'path': 'outputs/CNN/L1/model_l1.pth',
            'loss_type': 'l1',
            'base_visualizations_path': 'model_test/CNN/visualizations_l1'
        },
        'SimpleCNN_MSELoss_Model': {
            'path': 'outputs/CNN/MSE/model_mse.pth',
            'loss_type': 'mse',
            'base_visualizations_path': 'model_test/CNN/visualizations_mse' 
        },
        'SimpleCNN_BCE_Model_1': {
            'path': 'outputs/CNN/BCE/model_bcelogits.pth',
            'loss_type': 'bce',
            'base_visualizations_path': 'model_test/CNN/visualizations_bce' 
        },
        'SimpleCNN_BCE_Model_2': {
            'path': 'outputs/CNN/BCE2/model_bcelogits2.pth',
            'loss_type': 'bce',
            'base_visualizations_path': 'model_test/CNN/visualizations_bce' 
        },
        'SimpleCNN_MSELoss_Model2': {
            'path': 'outputs/CNN/MSE2/model_mse2.pth',
            'loss_type': 'mse',
            'base_visualizations_path': 'model_test/CNN/visualizations_mse' 
        },
    }
    ...
    for model_name, config in MODEL_CONFIGS.items():
        ...
        model_viz_output_dir = os.path.join(base_visualizations_path, current_timestamp)
        os.makedirs(model_viz_output_dir, exist_ok=True)
        
        print(f"\n--- Testing: {model_name} (Loss Type: {loss_type.upper()}) ---")
        print(f"Saving visualizations and model CSV to: {model_viz_output_dir}")
        ...
        try:
            ...
            model_metrics_csv_path = os.path.join(model_viz_output_dir, f"{model_name}_metrics.csv")
    ...
    overall_metrics_output_dir = os.path.join('model_test', 'CNN', 'overall_results', overall_output_timestamp)
    os.makedirs(overall_metrics_output_dir, exist_ok=True)
    overall_csv_path = os.path.join(overall_metrics_output_dir, 'simplecnn_all_models_heatmap_evaluation_metrics.csv')
    metrics_df.to_csv(overall_csv_path, index=False)
    print(f"\nOverall heatmap evaluation metrics saved to {overall_csv_path}")

    metrics_plot_output_dir = overall_metrics_output_dir 
    metrics_plot_filename = os.path.join(metrics_plot_output_dir, 'simplecnn_all_models_mae_3d_bar_chart.png')
